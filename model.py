...
def build_resnet(input_shape=(32, 32, 3), num_classes=10, stage_blocks=[3, 3, 3], stage_filters=[16, 32, 64], weight_decay=1e-4):
    inputs = layers.Input(shape=input_shape)

    # Initial Convolution
    x = layers.Conv2D(stage_filters[0], kernel_size=3, strides=1, padding='same', 
                      use_bias=False, kernel_regularizer=regularizers.l2(weight_decay))(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
